...
prefectures = ['kanagawa', 'saitama', 'chiba', 'ibaraki', 'gunma', 'oosaka',
                'hyogo', 'kyoto', 'shiga', 'nara', 'wakayama', 'aichi', 'shizuoka',
                'gifu', 'mie', 'hokkaidou', 'miyagi', 'fukushima', 'yamagata',
                'iwate', 'aomori', 'akita', 'nagano', 'niigata', 'yamanashi',
                'toyama', 'ishikawa', 'fukui', 'hiroshima', 'okayama', 'yamaguchi',
                'shimane', 'tottori', 'kagawa', 'ehime', 'tokushima', 'kouchi',
                'fukuoka', 'kumamoto', 'kagoshima', 'nagasaki', 'ooita', 'saga',
                'miyazaki', 'okinawa']
prefectures.each do |pref|
  natto = Natto::MeCab.new
  doc = Nokogiri::HTML(open("https://townwork.net/#{pref}"))
        ...
        if i == 0 # area code
          dls.css('option').each do |area|
            if area.inner_text != "選択してください"
              area_to_code[area.inner_text] = area["value"][1..-1] if area["value"][0] == 'S'
            end
          end
        end
  ...
  CSV.open("town_work_#{pref}.csv", 'w') do |csv|
    csv << csv_header
    num_sub_area = area_to_code.length
    sub_area_count = 0
    area_to_code.each do |area_name, area_code|
      sub_area_count += 1
      puts area_name
      puts "Now working on #{sub_area_count} / #{num_sub_area}"
            ...
            while true do
              ...
              doc.css('.job-lst-main-cassette-wrap .job-lst-box-wrap > a').each do |article| # 1 article
                ...
                if should_add_to_csv
                  csv << [pref, area_name, jc_name, jmc_name, baito, part, haken,
